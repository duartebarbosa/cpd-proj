...
int main(int argc, char** argv){
	...
	start = MPI_Wtime();
	ret |= MPI_Comm_size(MPI_COMM_WORLD, &numtasks);	/* total number of tasks */
	ret |= MPI_Comm_rank(MPI_COMM_WORLD, &taskid);		/* my TaskID */
	ret |= MPI_Get_processor_name(hostname, &len);		/* my computer's name */
	...
	proj_info[0] = info.document;
	proj_info[1] = info.subject;
	proj_info[2] = info.cabinet;

	MPI_Bcast(&proj_info[0], 3, MPI_INT, MASTER, MPI_COMM_WORLD);
